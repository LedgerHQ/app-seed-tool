...
const bagl_element_t screen_onboarding_restore_word_select_elements[] = {
    ...
    {{BAGL_LABELINE, 0x21, 0, 29, 128, 32, 0, 0, 0, 0xFFFFFF, 0x000000,
      BAGL_FONT_OPEN_SANS_REGULAR_11px | BAGL_FONT_ALIGNMENT_CENTER, 0},
     G_ux.string_buffer},
    {{BAGL_LABELINE, 0x20, 0, 43, 128, 32, 0, 0, 0, 0xFFFFFF, 0x000000,
...
void screen_onboarding_restore_word_display_auto_complete(void) {
    ...
    // prepare title of the common keyboard component, after the list of
    // possible letters
    (G_bolos_ux_context.tool_type == TOOL_TYPE_BIP39)
    ...
    G_bolos_ux_context.common_label =
        G_ux.string_buffer + 32 + auto_complete_count + 1;
    ...
    screen_common_keyboard_init(
        ...
        (strlen(G_ux.string_buffer + 16)
             ? 0
             : cx_rng_u8() %
                   auto_complete_count), /* start from a random element in the
                                            list for the word start letter, else
                                            keep the order */
        ...
        auto_complete_count + (strlen(G_ux.string_buffer + 16)
                                   ? 1
                                   : 0) /* backspace if a stem is already
                                           entered, else no backspace */
        ,
            ...
            if (G_ux.string_buffer[32 + value] == '\b') {
                ...
                if (G_bolos_ux_context.onboarding_words_checked >
                    ...
                    bolos_ux_hslider3_init(
                        G_bolos_ux_context.onboarding_words_checked + 1);
            ...
            if (value < 8) {
                ...
                if (l < 8) {
                    ...
                    if (value == 0) {
                        ...
                        G_ux.string_buffer[0] =
                            (G_ux.string_buffer + 16 + l + 1 - 8)[value];
                    }
                }
                break;
            }
        ...
        case 0x32:
            if (G_bolos_ux_context.onboarding_step == 0 ||
                G_bolos_ux_context.onboarding_step ==
                    G_bolos_ux_context.bip39_type - 1) {
                return NULL;  // don't display
            }
            break;
        case 0x33:
    ...
    if (G_bolos_ux_context.tool_type == TOOL_TYPE_BIP39) {
        if (G_bolos_ux_context.onboarding_step ==
            ...
            if (!valid) {
                ...
                // alright, the recovery phrase looks ok, compare it to
                // onboarded seed
...
unsigned int screen_onboarding_restore_word_intro_button(
    unsigned int button_mask,
    unsigned int button_mask_counter __attribute__((unused))) {
    switch (button_mask) {
        case BUTTON_EVT_RELEASED | BUTTON_LEFT | BUTTON_RIGHT: {
...
void screen_onboarding_restore_word_init(unsigned int firstWord) {
    ...
    G_ux.stack[0].button_push_callback =
        screen_onboarding_restore_word_intro_button;
    G_ux.stack[0].screen_before_element_display_callback =
        screen_onboarding_restore_word_before_element_display_callback;

    // elements to be displayed
    G_ux.stack[0].element_arrays[0].element_array =
        screen_onboarding_restore_word_intro_elements;
    G_ux.stack[0].element_arrays[0].element_array_count =
