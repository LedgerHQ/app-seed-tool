...
const bagl_element_t screen_onboarding_restore_word_select_elements[] = {
    ...
    {{BAGL_LABELINE, 0x01, 0, 12, 128, 32, 0, 0, 0, 0xFFFFFF, 0x000000,
     ...
     G_ux.string_buffer},
    {{BAGL_RECTANGLE, 0x02, 32, 16, 64, 14, 0, 4, BAGL_FILL, 0xFFFFFF, 0x000000,
      0, 0},
     NULL},
    {{BAGL_LABELINE, 0x02, 0, 26, 128, 32, 0, 0, 0, 0x000000, 0xFFFFFF,
...
void screen_processing_postinit(unsigned int stack_slot) {
    // ensure when pin is modal over the processing screen( at end of install)
    // the processing screen gives back the hand to the calling code
...
const bagl_element_t* screen_onboarding_restore_word_keyboard_callback(
...
void screen_onboarding_restore_word_display_auto_complete(void) {
    ...
    screen_common_keyboard_init(
        ...
        // (strlen(G_ux.string_buffer+16)?0:cx_rng_u8()%auto_complete_count), /*
        // start from a random element in the list for the word start letter,
        // else keep the order */ recompute alphabet and set the number of
        // elements in the keyboard
        auto_complete_count + (strlen(G_ux.string_buffer + 16)
                                   ? 1
                                   : 0) /* backspace if already a stem entered,
                                           else no backspace */
        ,
...
void screen_onboarding_restore_word_display_word_selection(void) {
    ux_stack_init(0);
    G_ux.stack[0].button_push_callback =
        screen_onboarding_restore_word_select_button;
    G_ux.stack[0].element_arrays[0].element_array =
        screen_onboarding_restore_word_select_elements;
        ...
        case KEYBOARD_ITEM_VALIDATED:
            ...
            if (G_ux.string_buffer[32 + value] == '\b') {
                ...
                if (G_bolos_ux_context.onboarding_words_checked >
                    ...
                    // account for the extra clear word, and clear any previous
                    // word items (go back in the onboarding process)
            ...
            if (G_ux.string_buffer[32 + value] == '\b') {
                ...
                */
                ...
                G_ux.string_buffer[0] =
                    G_ux.string_buffer[32 +
                                       value];  // render as lowercase, always
        ...
        case KEYBOARD_RENDER_WORD: {
            ...
            if (value < 8) {
                ...
                if (value < strlen(G_ux.string_buffer + 2)) {
                    ...
                    G_ux.string_buffer[0] =
                        G_ux.string_buffer[16 + value -
                                           strlen(G_ux.string_buffer + 2)];
                } else {
    ...
    if (G_bolos_ux_context.tool_type == TOOL_TYPE_BIP39) {
        if (G_bolos_ux_context.onboarding_step ==
            ...
            if (G_bolos_ux_context.onboarding_algorithm ==
                ...
                valid = bolos_ux_bip39_mnemonic_check(
                    (unsigned char*)G_bolos_ux_context.words_buffer,
                    G_bolos_ux_context.words_buffer_length);
            }
#else
            ...
            // enter the next word
            // indexes have been preincremented, it's therefore the next word
            // we're reentering
            screen_onboarding_restore_word_init(
                RESTORE_WORD_ACTION_REENTER_WORD);
        }
    } else if (G_bolos_ux_context.tool_type == TOOL_TYPE_SSKR) {
        if (G_bolos_ux_context.onboarding_step ==
            G_bolos_ux_context.bip39_type) {
            ...
            if (G_bolos_ux_context.sskr_share_index <
                ...
                // enter the next word
                // indexes have been preincremented, it's therefore the next
                // word we're reentering
                screen_onboarding_restore_word_init(
                    RESTORE_WORD_ACTION_REENTER_WORD);
            } else {
                unsigned int valid;
                valid = bolos_ux_sskr_hex_check(
...
unsigned int screen_onboarding_restore_word_select_button(
    ...
    switch (button_mask) {
        ...
        case BUTTON_EVT_RELEASED | BUTTON_LEFT | BUTTON_RIGHT:
            if (G_bolos_ux_context.hslider3_current <
                G_bolos_ux_context.onboarding_words_checked) {
                // confirm word, and prepare entering a new one or validate the
                // seed
                ...
                // current word IS NOT already stored. need to wipe one less
                // word example selecting word #2: 0         1         2 3
                //                            prop1     prop2     edit #2   edit
                //                            #1
                //                                                delete=0
                //                                                delete=1
                unsigned int word_to_delete =
                    G_bolos_ux_context.hslider3_current -
                    G_bolos_ux_context.onboarding_words_checked;
                // remove x words
                while (word_to_delete--) {
                    if (G_bolos_ux_context.tool_type == TOOL_TYPE_BIP39) {
                        if (G_bolos_ux_context.onboarding_step &&
                            G_bolos_ux_context.words_buffer_length) {
                            // remove the last space and up to the previous
                            // space (but keep the previous space)
                            do {
                            ...
                            while (
                                G_bolos_ux_context.words_buffer_length &&
                                G_bolos_ux_context
                                        .words_buffer[G_bolos_ux_context
                                                          .words_buffer_length -
                                                      1] != ' ');
