...
size_t sskr_shares_shrink(const size_t size) {
    ...
    memzero(&shares.buffer[shares.length],
            SSKR_SHARES_MAX_LENGTH - shares.length);
...
uint8_t sskr_sharenum_get(void) { return shares.group_descriptor[0][1]; }

void sskr_threshold_set(const uint8_t threshold) {
    shares.group_descriptor[0][0] = threshold;
}

uint8_t sskr_threshold_get(void) { return shares.group_descriptor[0][0]; }

uint8_t sskr_sharecount_get(void) { return shares.count; }

uint8_t sskr_shareindex_get(void) { return shares.current_share_index + 1; }
...
void sskr_shares_from_bip39_mnemonic(void) {
    ...
    bolos_ux_bip39_to_sskr_convert(
        (unsigned char*)bip39_mnemonic_get(), bip39_mnemonic_length_get(),
        bip39_mnemonic_final_size_get(), shares.group_descriptor[0],
        &shares.count, (unsigned char*)shares.buffer, &shares.length);
...
char* sskr_shares_get(void) { return shares.buffer; }

size_t sskr_shares_length_get(void) { return shares.length; }
#endif
